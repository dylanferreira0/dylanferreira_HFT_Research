...
public:
    ...
    void maintain_dom_levels(const RealMarketData& market_data) {
        ...
        // Get toxicity probability from predict_toxicity (for intensity calculation only)
        double toxicity_prob = predict_toxicity(market_data, true);  // For longs
        ...
        // O'HARA #8: Inventory-aware quote skewing and sizing
        double skewed_bid_price = inventory_control.get_skewed_bid_price(market_data.bid_px_00);
        ...
        // Symmetric placement: mirror long-side depth for shorts
        // NOTE: Gate is intentionally disabled - using "BLIND PLACEMENT" strategy
        // ML toxicity cancellation handles filtering instead of entry gates
        if (market_data.ask_px_00 > 0.0 && market_data.ask_px_00 >= 6000.0 && market_data.ask_px_00 <= 7000.0 && 
            ...
            //     continue;
            // }
            place_order_at_level(market_data.ask_px_00, thresholds.contracts_per_level, false, market_data);
    ...
    double predict_meta_probability(const RealMarketData& market_data, bool is_long) {
        ...
        double tox_prob = predict_toxicity(market_data, is_long);
    ...
    void check_toxicity_cancellation(const RealMarketData& market_data) {
        // Check toxicity for both sides separately
        double long_toxicity = predict_toxicity(market_data, true);   // Long side
        double short_toxicity = predict_toxicity(market_data, false); // Short side
